...
int main()
{
	...
	statFileUEP << "%Generation, Generation size, Symbol size, Layers, Layer 1 size, Layer 2 size, Layer 3 size, Received packets from layer 1, Received packets from layer 2, Received packets from layer 3, Total frames, Layer 1 Frames, Layer 2 Frames, Layer 3 Frames" << std::endl;
...
*/
	...
	for (int n = 0; n < Monte_Carlo; n++)
	{
		...
		for (Packet_Number = 0; Packet_Number < Over_Head * gsize; Packet_Number++)
		{
			...
			if (Packet_Number_1 == 0 && poo.is_layer_finish(1))
				Packet_Number_1 = Packet_Number+1;
			if (Packet_Number_2 == 0 && poo.is_layer_finish(2) && foo.get_layers() > 1)
				Packet_Number_2 = Packet_Number+1;
			if (Packet_Number_3 == 0 && poo.is_layer_finish(3) && foo.get_layers() > 2)
		...
		statFileUEP << n << ", " << gsize << ", " << symbol_size << ", " << foo.get_layers() << ", " << foo.get_layer_size(1) << ", " << foo.get_layer_size(2) << ", " << foo.get_layer_size(3) << ", " << Packet_Number_1 << ", " << Packet_Number_2 << ", " << Packet_Number_3 << ", " << total_buffers << ", " << frames_1 << ", " << frames_2 << ", " << frames_3 << std::endl;
